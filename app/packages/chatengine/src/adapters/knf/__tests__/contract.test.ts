...
import { describeTransportContract, fakeTransport, type ChatMessage, type TransportHarness } from '../../../index';
import { createKnfTransport, type ApiMessage, type HttpClient, type KnfSocketClient, type SocketEventName } from '..';
...
function knfHarness(): TransportHarness {
  ...
  let seq = 0;
  // A monotonic server clock, so a cursor taken between two writes
  // separates them even inside one millisecond
  let tick = 0;
  ...
  const http: HttpClient = {
    async get(path, options) {
      const pinsList = path.match(/^\/chat\/conversations\/([^/]+)\/pins$/);
      if (pinsList) {
        const conv = decodeURIComponent(pinsList[1]);
        const pins = rows
          .filter((r) => r.conversationId === conv && r.pinnedAt && !r.deleted)
          .sort((a, b) => ((a.pinnedAt as string) < (b.pinnedAt as string) ? 1 : -1));
        return { pins } as never;
      }
      const feed = path.match(/^\/chat\/conversations\/([^/]+)\/changes$/);
      if (feed) {
        const conv = decodeURIComponent(feed[1]);
        const since = String(options?.params?.since ?? '');
        const changed = rows.filter((r) => r.conversationId === conv && (updatedAt.get(r.id) ?? '') > since);
        return { messages: changed, cursor: now() } as never;
      }
      ...
      const mine = rows.filter((r) => r.conversationId === conv);
      const limit = Number(params.limit ?? 50);
      const shape = (messages: ApiMessage[], hasMore: boolean, hasNewer: boolean) =>
        ({ messages, hasMore, hasNewer, participants: [{ id: selfId, displayName: 'Me' }], conversation: { id: conv, type: 'direct', title: null, avatarEmoji: null }, cursor: now() }) as never;
      if (typeof params.around === 'string') {
        const anchor = mine.find((r) => r.id === params.around);
        if (!anchor) throw Object.assign(new Error('not found'), { status: 404, code: 'http' });
        const half = Math.max(1, Math.floor(limit / 2));
        const older = mine.filter((r) => r.createdAt < anchor.createdAt || (r.createdAt === anchor.createdAt && r.id < anchor.id));
        const newer = mine.filter((r) => r.createdAt > anchor.createdAt || (r.createdAt === anchor.createdAt && r.id > anchor.id));
        return shape([...older.slice(Math.max(0, older.length - (half - 1))), anchor, ...newer.slice(0, half)], older.length > half - 1, newer.length > half);
      }
      if (typeof params.after === 'string') {
        const after = params.after;
        const afterId = typeof params.after_id === 'string' ? params.after_id : '';
        const newer = mine.filter((r) => r.createdAt > after || (r.createdAt === after && r.id > afterId));
        return shape(newer.slice(0, limit), mine.length > newer.length, newer.length > limit);
      }
      let eligible = mine;
      if (typeof params.before === 'string') {
        const before = params.before;
        const beforeId = typeof params.before_id === 'string' ? params.before_id : '';
        eligible = eligible.filter((r) => r.createdAt < before || (r.createdAt === before && r.id < beforeId));
      }
      return shape(eligible.slice(Math.max(0, eligible.length - limit)), eligible.length > limit, false);
    },
    ...
    async put(path, body) {
      const pin = path.match(/^\/chat\/conversations\/([^/]+)\/messages\/([^/]+)\/pin$/);
      if (pin) {
        const row = rows.find((r) => r.id === decodeURIComponent(pin[2]));
        if (!row || row.deleted) throw Object.assign(new Error('not found'), { status: 404, code: 'http' });
        row.pinnedAt = now();
        row.pinnedBy = selfId;
        return { pinnedAt: row.pinnedAt, pinnedBy: row.pinnedBy } as never;
      }
      if (/\/ttl$/.test(path)) return { messageTtlSeconds: (body as { seconds?: number | null })?.seconds ?? null } as never;
      ...
      if (edit) {
        ...
        row.editedAt = now();
        updatedAt.set(row.id, row.editedAt);
