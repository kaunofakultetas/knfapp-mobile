...
//  Confirm is inert while the push is in flight. The card
//  rides the keyboard (a KeyboardAvoidingView inside the Modal
//  — a Modal is its own window, the screen's avoidance never
//  reaches it) and scrolls when a small phone has less room
//  than the card needs; the preview shrinks while typing, so
//  the fields and the buttons stay above the keys.
//
//  Used by:
...
export default function MemePushSheet({
  ...
  // Seeded from the pick even on a first render that already
  // carries one — keyed on the uri, re-seeded for every new pick
  const [title, setTitle] = useState(() => stemTitle(asset?.fileName));
  ...
  return (
    ...
    >
      <KeyboardAvoidingView behavior="padding" style={{ flex: 1, justifyContent: 'flex-end' }} testID="meme-push-avoid">
        ...
        <View
          className="mx-md rounded-2xl bg-surface"
          style={{ marginBottom: keyboardUp ? 12 : insets.bottom + 24, marginTop: insets.top + 12, maxHeight: '100%' }}
          accessibilityViewIsModal
          testID="meme-push-sheet"
        >
          <ScrollView keyboardShouldPersistTaps="handled" bounces={false} contentContainerStyle={{ padding: 16 }} testID="meme-push-scroll">
            ...
            {asset ? (
              <ExpoImage
                ...
                style={{ width: '100%', height: keyboardUp ? 88 : 160, borderRadius: 12, marginBottom: 12 }}
                contentFit="contain"
