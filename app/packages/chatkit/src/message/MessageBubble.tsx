...
//  at its natural proportions / the video poster with a play
//  disc / the document card, then the text with tappable
//  links. An unsent message renders a muted italic placeholder
//  in an outlined bubble. Reactions sit as small pills
//  overlapping the bubble's bottom edge; an edited message
//  carries an "edited" mark on its time line.
//
...
//                     context menu's floating copy); the parts
//                     live beside it: ReplyQuote, MessageText,
//                     attachments/{Image,Video}Attachment, FileCard
//    ReceiptLine    — status / time / edited under the bubble
...
// Used by:
//   - MessageBubble (below)
//   - chatkit/menu/MessageContextMenu.tsx — the floating copy
// -----------------------------------------------------------
...
export function BubbleBody({
  ...
  onLongPress?: () => void;
  // Reports the measured photo / poster ratio up, into the
  // context target
  ...
  const style: ViewStyle = {
    ...
    paddingHorizontal: media ? 0 : BUBBLE_PADDING_H,
    paddingVertical: media ? 0 : BUBBLE_PADDING_V,
  ...
  // The caption under a photo / video, or the whole body of a
  ...
  return (
    <View style={style}>
      {message.replyTo ? (
        <View style={media || file ? { paddingHorizontal: BUBBLE_PADDING_H, paddingTop: BUBBLE_PADDING_V } : undefined}>
      ...
      {photo ? (
        ...
        <Pressable
          ...
          onLongPress={onLongPress}
          delayLongPress={260}
          // Playable once the upload finished (the stored uri is
          // a server path by then); a temp keeps the tap for the
          // bubble's own handlers
          disabled={message.status === 'sending' || message.status === 'failed'}
          accessible={!!onPressVideo}
          accessibilityLabel={labels.video}
          accessibilityHint={onPressVideo ? labels.playVideo : undefined}
        >
          <VideoAttachment
            video={video}
            mediaSize={message.mediaSize}
...
function ReceiptLine({
  ...
  // An edit leaves a permanent mark on the line, even with the
  // time hidden and no delivery state to show
  const edited = !!message.editedAt && !message.deleted;
  if (!showStatus && !showTime && !edited) return null;
  ...
  const status =
    ...
    : labels.sent;
  const parts = [...(showTime ? [timeText] : []), ...(edited ? [labels.edited] : []), ...(showStatus ? [status] : [])];
...
//
// Used by:
//   - chatkit/list/MessageList.tsx — one per message row
// -----------------------------------------------------------
...
function MessageBubbleInner({
  ...
  const accessibilityActions = useMemo<AccessibilityActionInfo[]>(() => {
    ...
    if (kind === 'image' && message.imageUrl) list.push({ name: 'openPhoto', label: labels.photo });
    if (kind === 'video' && onPressVideo && message.status !== 'sending' && message.status !== 'failed') list.push({ name: 'openVideo', label: labels.playVideo });
    if (kind === 'file' && message.file) list.push({ name: 'openFile', label: `${labels.file}: ${message.file.name}` });
    ...
    return list;
  }, [deleted, actionable, swipeable, kind, onPressVideo, message.text, message.replyTo, message.imageUrl, message.file, message.status, links, labels]);
  ...
  return (
    <Animated.View
      // The inverted native cell mirrors Y, so 'Down' rises there;
      // the upright web list rises with 'Up'
      entering={animateIn && !reduceMotion ? (isWeb ? FadeInUp.duration(220) : FadeInDown.duration(220)) : undefined}
      style={{
        flexDirection: 'row',
        alignItems: 'flex-end',
    ...
    >
      ...
      <View style={{ maxWidth: BUBBLE_MAX_WIDTH, alignItems: own ? 'flex-end' : 'flex-start' }}>
        ...
        {showSender ? (
          <Text
            style={[text.name, { marginBottom: 3, marginLeft: BUBBLE_PADDING_H, color: colors.inkSoft }]}
            numberOfLines={1}
          >
            {message.senderName}
          </Text>
        ...
        {hasReactions || timeRevealed || (showStatus && !deleted) || message.status === 'failed' || (!!message.editedAt && !deleted) ? (
          <View
            style={{
