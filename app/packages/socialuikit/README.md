...
| Activity | grouped rows — the actor's portrait, or a group's first two people as a diagonal duo in the same one-portrait slot (every sentence starts on one line), the sentence built by the label functions ("Ona ir dar 3 žmonės pamėgo jūsų įrašą"), a one-line subject snippet; **unknown kinds degrade** to a generic line instead of crashing old clients; unread rows take the tint, the brand dot and a `-unread` testID |
