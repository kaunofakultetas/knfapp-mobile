...
jest.mock('@/services/api', () => ({ getUploadUrl: (path: string) => path }));
jest.mock('@/hooks/useTheme', () => {
  const { palettes } = jest.requireActual('@/constants/theme');
  return { useTheme: () => ({ scheme: 'light', colors: palettes.light }) };
});
jest.mock('@knf/chatuikit/provider', () => ({
  ...jest.requireActual('@knf/chatuikit/provider'),
  useKitLabels: () => mockKitLabels,
}));

import { render } from '@testing-library/react-native';

import MessageBubble from '@knf/chatuikit/message/MessageBubble';
import MessageList, { keyExtractor } from '@knf/chatuikit/list/MessageList';
import { buildTimeline } from '@knf/chatuikit/core/timeline';
