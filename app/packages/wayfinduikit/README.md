...
<FloorPlan level={level} plan={<SvgXml xml={drawing} width="100%" height="100%" />}
           route={route.floors}                          // every stretch — the shown floor's are drawn
...
| The overlay | bottom to top: room polygons (a faint brand wash, tappable, named), the route as a **glow under a line**, corridor nodes with a hit disc well beyond the visible dot, the start ring, the destination **pin**, the walker's dot (brand disc in a white ring over a soft halo); marker weights are given in screen pixels and turned into plan units, so a drawing of any resolution shows the same weight of line; `route` takes one segment or the route's whole list — every stretch on the shown floor is drawn as its own subpath, so a route that leaves a floor and **comes back** keeps its line to the pin, and a segment from **another level draws nothing**; the room layer is memoised, so a walker's step or a finger's drag does not re-render every polygon; the points carry their floor too — `start`, `end`, `youAreHere` and `focus` are `{ x, y, level? }`, a point naming another level is **neither drawn nor glided to** (a focus on another floor does not zoom) and one naming no level is taken as on the shown floor — so the host hands the plan whatever it holds, the walker's position and the route's ends included |
