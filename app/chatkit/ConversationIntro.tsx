...
//
//  Used by:
//    - chatkit/MessageList.tsx — when there is no older page
// -----------------------------------------------------------
...
export default function ConversationIntro({ title, subtitle, avatarUrl, isGroup, members }: IntroInfo) {
  ...
  return (
    <View className="items-center" style={{ paddingTop: 28, paddingBottom: 12 }}>
      ...
      <Text className="mt-3 font-raleway-bold text-lg text-ink" numberOfLines={1}>
        {title}
      </Text>
      <Text className="mt-1 font-raleway text-sm text-ink-soft" numberOfLines={1}>
        {subtitle}
      </Text>
    </View>
  );
