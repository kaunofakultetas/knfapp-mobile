// -----------------------------------------------------------
//  [*] @knf/assistantengine — jest
//
//  The package proves itself without a host: `npm test` here
//  runs every src/**/__tests__ spec with the jest-expo preset
//  (found through the host's node_modules) and this package's
//  own babel config. The host's root jest run picks the same
//  specs up too. The upstream runtime, its stream protocol
//  and their schema helpers ship as ES modules, so they are
//  whitelisted for transformation — the same additions the
//  host's root config carries.
// -----------------------------------------------------------

module.exports = {
  preset: 'jest-expo',
  globalSetup: '<rootDir>/jest.globalSetup.js',
  // Relative to this file — jest resolves a config's rootDir against its own directory
  rootDir: '.',
  testMatch: ['<rootDir>/src/**/__tests__/**/*.test.ts?(x)'],
  transformIgnorePatterns: [
    'node_modules/(?!((jest-)?react-native|@react-native(-community)?)|expo(nent)?|@expo(nent)?/.*|@expo-google-fonts/.*|react-navigation|@react-navigation/.*|native-base|react-native-svg|nativewind|react-native-css-interop|standard-navigation|@assistant-ui/.*|assistant-stream|assistant-cloud|ai/|@ai-sdk/.*|zod|nanoid|@standard-schema/.*|@workflow/.*|eventsource-parser|@vercel/oidc|pkce-challenge)',
  ],
  collectCoverageFrom: ['<rootDir>/src/**/*.{ts,tsx}', '!<rootDir>/src/**/__tests__/**'],
};
