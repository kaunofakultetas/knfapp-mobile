...
A reusable messaging kit for Expo / React Native — the iMessage /
...
```tsx
import { ChatKitProvider, defaultLabels } from '@knf/chatkit';
...
`components/chat/ChatKitHost.tsx`, which maps the KNF palette, Raleway,
...
| `provider/` | The host seam: `ChatKitProvider` + the `useKit*` hooks (`index.tsx`), `theme.ts`, `labels.ts` |
...
`index.ts` is the public surface; deep imports (`@knf/chatkit/core/timeline`)
...
Resolution: the app aliases `@knf/chatkit` to `packages/chatkit/src` in
