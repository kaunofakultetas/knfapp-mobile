...
export function buildTimeline(
  ...
  const sameRun = (newer: KitMessage | undefined, older: KitMessage | undefined) => {
    ...
    // A system row is never part of anyone's run
    if (messageKind(newer) === 'system' || messageKind(older) === 'system') return false;
  ...
  for (let i = 0; i < messages.length; i++) {
    ...
    // The unread line sits ABOVE the first unread row: in this
    // newest-first array that means right after it
    if (options.unreadFromId && message.id === options.unreadFromId) {
...
// -----------------------------------------------------------
//
// The day label the floating date pill shows for the row at the
// top of the viewport: a separator's own day, a message's day
// (Today / Yesterday / weekday / date), nothing for the unread
// line. Pure; MessageList calls it from the viewability callback.
//
// Used by:
//   - MessageList.tsx
// -----------------------------------------------------------

export function floatingDayFor(item: TimelineItem, labels: TimelineLabels): string {
  if (item.type === 'separator') return item.day || item.time;
  if (item.type === 'message') return dayLabel(item.message.createdAt, labels) || labels.today;
  return '';
}
